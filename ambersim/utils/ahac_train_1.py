...
def train(
    ...
    transition_buffer = []
    # transition_buffer.append(dummy_transition)

    def gradient(fn, mode, args=None):
            if mode == "jax_grad":
                # args = {"idx": idx of argument gradien is respect to}
                # import ipdb; ipdb.set_trace()
                return jax.grad(fn)(args["idx"], args["idx"], gamma=0.1)
            elif mode == "finite_diff":
                # args = {"curr": current args, 
                #         "prev": previous args}
                # partial / dim
                return (fn(args["curr"]) - fn(args["prev"])) / (args["curr"] - args["prev"])
            elif mode == "jacfwd":
                # args = {"idx": idx of argument gradient is respect to
                return jax.jacfwd(fn)(args["idx"])
            else:
                print(f"Gradient calculation using {mode} is not supported.")
    ...
    C = 500 # contact threshold
    f_grad = lambda i: gradient(state_buffer[i])
    def loss_policy(theta, phi, gamma):
        t = 1
        # import ipdb; ipdb.set_trace()
        return jnp.sum(jnp.array([gamma**(h-t) * reward_buffer[h] for h in range(t, t+H-1)])) \
                + gamma**H * V(state_buffer[t+H]) \
                +  phi.T @ (C - f_grad)
    # t = 1
    lam = ...
    R_h = lambda s_t, t, h : jnp.sum([gamma**(n-t) * reward_buffer[n] + gamma**(t+h) * V(state_buffer[t+h]) for n in range(t, t+h-1)])
    R_H = lambda s_t, t : jnp.sum(reward_buffer[t:H])
    V_hat = lambda s_t, t : (1-lam) * (sum([lam**(h-1)*R_h(s_t,t,h) for h in range(1, H-T-1)])) \
                            + lam**(H-t-1) * R_H(s_t, t)
    # import ipdb; ipdb.set_trace()
    V_V = V(state_buffer) - V_hat(state_buffer,t)
    loss_V = jnp.sum((V_V / jnp.linalg.norm(V_V, axis=0, keepdims=True)[0])[t:t+H])
    ...
    def sgd_step(
        ...
        phi = jnp.zeros(T)
        H = 1
        theta = theta + alpha_theta * gradient(loss_policy, grad_mode, args)[0]
        # Actor phi update
        phi = phi - alpha_phi * gradient(loss_policy, grad_mode, args)[1]

        # TODO: add H(orizon) update function
        # Horizon update
        H = H + alpha_phi * jnp.sum(phi)
    ...
    def get_experience(
        ...
        transition_buffer.append(transitions)
    ...
    def training_step(
        ...
        state_buffer.append(buffer_state)
